...
function param_solver(I, Δt, c₂ₙₚ, c₂ₛₚ, c₂ₚₚ, c₂ₙ₀ₚ, c₂ₚₗₚ, jn_0, jp_l )
    ...
    # Eqn 82 & 83
    A[11, 3] = 2 * d₂ₙ(c₂ₙₚ) * Δt/ϵ₂ₙ
    A[11, 4] = 3 * lₙ * d₂ₙ(c₂ₙₚ) * Δt/ϵ₂ₙ
    A[11, 12] = 1
    # A[11, :] *= Δt/ϵ₂ₙ

    A[12, 7] = 2 * d₂ₚ(c₂ₚₚ) * Δt/ϵ₂ₚ
    A[12, 8] = 3 * lₚ * d₂ₚ(c₂ₚₚ) * Δt/ϵ₂ₚ
    A[12, 13] = 1
    # A[12, :] *= Δt/ϵ₂ₚ

    # Eqn 84 & 85
    A[13, 3] = 2*d₂ₙ(c₂ₙₚ)*Δt/ϵ₂ₙ; A[13, 15] = 1
    # A[13, :] *= Δt/ϵ₂ₙ

    A[14, 7] = 2*d₂ₚ(c₂ₚₚ)*Δt/ϵ₂ₚ; A[14, 16] = 1
    # A[14, :] *= Δt/ϵ₂ₚ
