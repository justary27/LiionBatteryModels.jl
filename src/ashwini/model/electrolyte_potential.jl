...
function ϕ₂ₙ(x, I, params, args...)
    ...
    ϕ₂ᵢₙ(I, params) + 2*θ*log(c₂(x, params)./c₂ᵢₙ(params)) + I*(lₙ-x)/k₂ₙ(params) - I*exp(dₙ₁)/((k₂ₙ(params))*(exp(dₙ₁) - 1))*((lₙ-x)-lₙ*(1-exp(-dₙ₁+dₙ₁*x/lₙ))/dₙ₁)
end
...
function ϕ₂ₚ(x, I, params, args...)
    ...
    ϕ₂ᵢₙ(I, params) + 2*θ*log(c₂(x, params)./c₂ᵢₚ(params)) + I*(lₙ+lₛ-x)/(2*k₂ₚ(params)) + I*exp(dₚ₁)/((k₂ₚ(params))*(exp(dₚ₁) - 1))*((x-lₙ-lₛ)+lₚ*(-1+exp(dₚ₁*(x-lₙ-lₛ)/lₚ))/dₚ₁)
end
